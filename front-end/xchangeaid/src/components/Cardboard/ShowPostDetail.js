...
toast.configure();
class ShowPostDetail extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            comment: "",
    ...
    onDelete() {
        let idpost = this.props.match.params.id;
        axios.delete('https://heroku-xchangeaid.herokuapp.com/deleteArticle/' + idpost)
            .then(response => {
                this.props.history.push('/Home');
            })
            .catch(err => console.log(err));
            toast.warn('Delete Article Successfully!', {
    ...
    onSubmit = () => {
        let idpost = this.props.match.params.id;
        this.props.history.push('/EditReadmore/' + idpost);
    }
            ...
            <Container className="main-wrapper" >
                <Form>
                    ...
                    <Form.Group controlId="exampleForm.ControlTextarea1">
                        <h2>Content</h2>
                        {this.state.details.map((detail) => (
                            <Form.Label key={detail.articleContent}>{detail.articleContent}</Form.Label>
                        ))}
                    </Form.Group>
                    <Form.Group>
